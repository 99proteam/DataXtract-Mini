...
test.before(async () => {
    serverProcess = spawn('node', ['server.js'], {
        ...
        env: {
            ...
            DATAXTRACT_SETTINGS_FILE: path.join(root, 'tests', 'fixtures', 'unconfigured-settings.json')
        },
        stdio: 'ignore'
    });
...
test('clicking the AI Configure link opens the AI settings tab (not API Keys)', async () => {
    ...
    // Wait for the asynchronous readiness request to replace loading placeholders.
    await page.waitForFunction(() => Array.from(document.querySelectorAll('.integration-badge'))
        .some(badge => badge.textContent.includes('AI') && badge.querySelector('button')));

    // find and click the "Configure" button inside the AI badge
    const clicked = await page.evaluate(() => {
        ...
        const link = aiBadge.querySelector('button');
        if (!link || link.textContent.trim() !== 'Configure') return false;
        link.click();
        return true;
    });
